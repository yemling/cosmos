...
cosmos.markerCSS = function (bgColor) {
    var markerHtmlStyles = 'background-color: ' + bgColor + ';';
    markerHtmlStyles += 'background-color: -moz-radial-gradient(center, ellipse cover, ' + bgColor + ' 50%, ' + bgColor + ' 50%, #000000 100%)';
    markerHtmlStyles += 'background-color: -webkit-radial-gradient(center, ellipse cover, ' + bgColor + ' 50%, ' + bgColor + ' 50%,#000000 100%);';
    markerHtmlStyles += 'background-color: radial-gradient(ellipse at center, ' + bgColor + ' 50%,' + bgColor + ' 50%,#000000 100%);';
    ...
    markerHtmlStyles += 'background-color: radial-gradient(circle,' + bgColor + ',#ddd);';
...
cosmos.outputSiteData = function (siteData) {
    ...
    for (c = 0; c < markerColors.length; c++) {
        ...
        jQuery('#vwcKey').append('<div><span class="leftColTwoThirds">' + colorKey.key + ': </span><span class="floatLeft" style="' + cosmos.markerCSS(colorKey.color) + '"></span><br clear="all" /></div>');
    }
    //Uncalibrated
    jQuery('#vwcKey').append('<div><span class="leftColTwoThirds">Uncalibrated: </span><span class="floatLeft" style="' + cosmos.markerCSS('rgb(0,0,0)') + '"></span></div>');
    for (i = 0; i < sites.length; i++) {
        ...
        if (site.id != 'JUNG') {
            ...
            jQuery('#siteDetailTable').append('<tr><td><span class="siteCell" rel="' + site.id + '" title="Highlight ' + site["name"] + ' on the map" style="' + cosmos.markerCSS(markerBackgroundColor) + '">&nbsp;&nbsp;&nbsp;</span></td><td><a href="/sites/' + site.id + '" title="View details about ' + siteLocation + '">' + siteLocation + '.</a></td></tr>');
